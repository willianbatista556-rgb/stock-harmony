...
import {
  ...
  AlertTriangle, ArrowDownCircle, ArrowUpCircle, Equal, XCircle, Loader2,
} from 'lucide-react';
...
export default function InventarioContagem() {
  ...
  const status = inventario?.status || 'em_andamento';
  const isContagem = status === 'em_andamento';
  const isFinalizado = status === 'finalizado';
  const isAplicado = status === 'aplicado';

  const depositoNome = inventario ? depositos.find(d => d.id === inventario.deposito_id)?.nome || '—' : '—';

  // Map produto_id → Produto for quick lookup
  ...
  const handleAddProduto = async (produto: Produto) => {
    ...
    await addItem.mutateAsync({ inventarioId, produtoId: produto.id, qtd });
    setBusca('');
    setQtdInput('1');
  ...
  const totalContado = itens.reduce((s, i) => s + i.qtd_contada, 0);
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-display font-bold text-foreground">
              {isContagem ? 'Contagem Cega' : isFinalizado ? 'Divergências' : 'Inventário'}
            </h1>
            <p className="text-sm text-muted-foreground">
              Depósito: {depositoNome} · {totalItens} produtos contados
            </p>
          </div>
        </div>
        ...
        {itensLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 gap-3">
            ...
            <p className="text-muted-foreground">
              {isContagem ? 'Comece bipando ou buscando um produto acima.' : 'Nenhum item contado.'}
            </p>
          </div>
        ) : (
          <Table>
            <TableHeader>
              <TableRow className="bg-muted/50">
                ...
                {!isContagem && (
                  <>
                    <TableHead className="font-semibold text-right">Sistema</TableHead>
      ...
      <Dialog open={showFinalizar} onOpenChange={setShowFinalizar}>
        <DialogContent className="bg-card max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              O sistema vai comparar as quantidades contadas com o estoque atual e calcular as divergências. Esta ação não altera o estoque.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            ...
            <Button onClick={handleFinalizar} disabled={finalizar.isPending} className="gradient-primary text-primary-foreground">
              {finalizar.isPending ? <Loader2 className="w-4 h-4 animate-spin mr-1.5" /> : null}
              Finalizar
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={showAplicar} onOpenChange={setShowAplicar}>
        <DialogContent className="bg-card max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              O estoque será ajustado para refletir as quantidades contadas. {divergencias} divergência(s) serão corrigidas com movimentações de ajuste.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            ...
            <Button onClick={handleAplicar} disabled={aplicar.isPending} className="gradient-primary text-primary-foreground">
              {aplicar.isPending ? <Loader2 className="w-4 h-4 animate-spin mr-1.5" /> : null}
              Aplicar
            </Button>
          </DialogFooter>
        </DialogContent>
